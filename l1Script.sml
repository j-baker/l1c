...
val (bs_type_rules, bs_type_ind, bs_type_cases) = Hol_reln `
    (!g.bs_type (B_Value B_Skip) g unitL1) /\
    (!n g.bs_type (B_Value (B_N n)) g intL1) /\
    (!b g.bs_type (B_Value (B_B b)) g boolL1) /\

    (!e1 e2 g.
       (bs_type e1 g intL1 /\
        bs_type e2 g intL1)
    ==> bs_type (B_Plus e1 e2) g intL1) /\

    (!e1 e2 g.
       (bs_type e1 g intL1 /\
        bs_type e2 g intL1)
    ==> bs_type (B_Geq e1 e2) g boolL1) /\
    ...
    (!e1 e2 e3 g t.
       (bs_type e1 g boolL1 /\
        bs_type e2 g t /\
        bs_type e3 g t)
    ==> bs_type (B_If e1 e2 e3) g t) /\

    (!l e g.
       (l ∈ g /\
        bs_type e g intL1)
    ==> bs_type (B_Assign l e) g unitL1) /\

    (!l g.
        l ∈ g
    ==> bs_type (B_Deref l) g intL1) /\

    (!e1 e2 g t.
       (bs_type e1 g unitL1 /\
        bs_type e2 g t)
    ==> bs_type (B_Seq e1 e2) g t) /\

    (!e1 e2 g.
       (bs_type e1 g boolL1 /\
        bs_type e2 g unitL1)
    ==> bs_type (B_While e1 e2) g unitL1)`;

val bs_type_strongind = derive_strong_induction(bs_type_rules, bs_type_ind);
