...
THEN rw [Once bs_il1_expr_cases]
THEN fs [equiv_def, con_store_def, MAP_KEYS_def, STORE_L1_IL1_INJ])
(* End dereference case *)

(* Begin assign case *)
THEN1 (fs [l1_to_il1_pair_def, l1_il1_val_def] THEN rw []
THEN `?st1 ex1 lc1''.l1_to_il1_pair lc1 e = (st1, ex1, lc1'')` by metis_tac [L1_TO_IL1_TOTAL_THM]
THEN fs [LET_DEF] THEN rw []
...
(* Begin seq case *)
THEN1 (fs [l1_to_il1_pair_def, l1_il1_val_def]
...
THEN `?st1 ex1 lc1''.l1_to_il1_pair lc1 e1 = (st1, ex1, lc1'')` by metis_tac [L1_TO_IL1_TOTAL_THM]
THEN `?st2 ex2 lc2''.l1_to_il1_pair lc1'' e2 = (st2, ex2, lc2'')` by metis_tac [L1_TO_IL1_TOTAL_THM]
THEN `?st3 ex3 lc3''.l1_to_il1_pair lc2'' e3 = (st3, ex3, lc3'')` by metis_tac [L1_TO_IL1_TOTAL_THM]
THEN fs [LET_DEF] THEN rw []
...
THEN `?st3 ex3 lc3''.l1_to_il1_pair lc2'' e1 = (st3, ex3, lc3'')` by metis_tac [L1_TO_IL1_TOTAL_THM]

THEN fs [LET_DEF] THEN rw []

THEN `?fs'.bs_il1 (st1,fs) IL1_ESkip fs' /\ bs_il1_expr (ex1, fs') (l1_il1_val (B_B T)) /\ equiv (con_store s') fs'` by metis_tac []
THEN `?fs''.bs_il1 (st2,fs') IL1_ESkip fs'' /\ bs_il1_expr (ex2, fs'') (l1_il1_val (B_Skip)) /\ equiv (con_store s'') fs''` by metis_tac []
THEN rw []
THEN res_tac
...
THEN1 (fs [l1_to_il1_pair_def] THEN rw []
...
THEN`?fs'.bs_il1 (st1, fs) IL1_ESkip fs' /\ bs_il1_expr (ex1, fs') (l1_il1_val (B_B F)) /\ equiv (con_store s') fs'` by metis_tac []
...
recInduct (fetch "-" "l1_to_il1_pair_ind") THEN rw [] THEN fs [l1_to_il1_pair_def]
 THEN rw []
THEN `?stjj exjj ctjj.l1_to_il1_pair lc e = (stjj, exjj, ctjj)` by metis_tac [L1_TO_IL1_TOTAL_THM]
...
THEN fs [(fetch "l1" "bs_type_fun_def"), il1_type_def, il1_type_pair_def, il1_expr_type_def, LET_DEF]
THEN rw []
THEN fs [con_store_def, FDOM_DEF, MAP_KEYS_def]
THEN rw [il1_type_pair_def, LET_DEF]
THEN res_tac

THEN1 (Cases_on `il1_type_pair stjj (IMAGE User (FDOM s))` THEN fs []
THEN Cases_on `x` THEN fs []
THEN Cases_on `q` THEN fs []
THEN Cases_on `il1_expr_type exjj r` THEN fs [] THEN rw [il1_type_pair_def, il1_expr_type_def])

THEN1 (Cases_on `il1_type_pair st' (IMAGE User (FDOM s))` THEN fs []
THEN Cases_on `x` THEN fs []
THEN Cases_on `q` THEN fs []
THEN Cases_on `il1_type_pair st'' (IMAGE User (FDOM s))` THEN fs [] THEN rw [il1_type_pair_def, il1_expr_type_def]
...
THEN `?st ex ct''.l1_to_il1_pair ct1 e3 = (st, ex, ct'')` by metis_tac [L1_TO_IL1_TOTAL_THM]
THEN fs [LET_DEF] THEN rw []
THEN fs [(fetch "l1" "bs_type_fun_def")] THEN rw []
THEN res_tac
THEN fs [il1_type_def, il1_type_pair_def, il1_expr_type_def, LET_DEF]

THEN Cases_on `il1_type_pair stjj (FDOM (con_store s))` THEN fs []
THEN Cases_on `x` THEN fs []
THEN Cases_on `q` THEN fs []
THEN Cases_on `il1_expr_type exjj r` THEN fs [])
